...
if(document.querySelector('.slider-castoms__body')) {
	new Swiper('.slider-castoms__body', {
		observer: true,
		observeParents: true,
		slidesPerView: 'auto',
		spaceBetween: 24,
		...
		pagination: {
			...
			// Arrows
			navigation: {
				nextEl: '.slider-castoms .slider-arrow_next',
				prevEl: '.slider-castoms .slider-arrow_prev',
			}
...
if(document.querySelector('.slider-decor__body')) {
	new Swiper('.slider-decor__body', {
		...
		pagination: {
			...
			// Arrows
			navigation: {
				nextEl: '.slider-decor .slider-arrow_next2',
				prevEl: '.slider-decor .slider-arrow_prev2',
			}
...
for (let index = 0; index < popup_link.length; index++) {
	...
	el.addEventListener('click', function (e) {
		if (unlock) {
			...
			let dataId = el.getAttribute('data-pid');
			popup_open(item, dataId );
		}
		e.preventDefault();
	})
}
...
function popup_open(item, dataId = '') {
	let activePopup = document.querySelectorAll('.popup._active');
	if (activePopup.length > 0) {
		popup_close('', false);
	}
	const popupQuerySelector = dataId ? '.popup_' + item + '_' + dataId : '.popup_' + item

	console.log(popupQuerySelector)
	let curent_popup = document.querySelector(popupQuerySelector);
